...
void World::generateBridge(float x, float y, float rotation) {
    ...
    bridge.x = x;
    bridge.y = y;
    bridge.z = 30.0f;  // Bridge deck height
    bridge.radius = 25.0f;
    bridge.height = 20.0f;
...
void World::generateTunnel(float x, float y, float rotation) {
    ...
    tunnel.x = x;
    tunnel.y = y;
    tunnel.z = 15.0f;  // Tunnel entrance height
    tunnel.radius = 20.0f;
    tunnel.height = 30.0f;
...
void World::generateArch(float x, float y, float rotation) {
    ...
    arch.x = x;
    arch.y = y;
    arch.z = 25.0f;  // Arch center height
    arch.radius = 15.0f;
    arch.height = 50.0f;
...
void World::generatePyramid(float x, float y) {
    ...
    pyramid.x = x;
    pyramid.y = y;
    pyramid.z = 40.0f;  // Pyramid center height
...
void World::generateSphereBuilding(float x, float y) {
    ...
    sphere.x = x;
    sphere.y = y;
    sphere.z = 35.0f;  // Sphere center height
    sphere.radius = 25.0f;
    sphere.height = 70.0f;
...
void World::generateWindTurbine(float x, float y) {
    ...
    turbine.x = x;
    turbine.y = y;
    turbine.z = 60.0f;  // Turbine hub height
    turbine.radius = 5.0f;
...
void World::generateRadioTower(float x, float y) {
    ...
    tower.x = x;
    tower.y = y;
    tower.z = 80.0f;  // Tower center height
    tower.radius = 8.0f;
    tower.height = 160.0f;
...
void World::generateWaterTower(float x, float y) {
    ...
    waterTower.x = x;
    waterTower.y = y;
    waterTower.z = 45.0f;  // Tank center height
...
void World::generateFactory(float x, float y, float rotation) {
    ...
    factory.x = x;
    factory.y = y;
    factory.z = 25.0f;  // Factory center height
